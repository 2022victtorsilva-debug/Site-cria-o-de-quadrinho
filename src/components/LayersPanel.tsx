...
export function LayersPanel({ api, state, onClose, embedded = false }: { api: CanvasEditorApi | null; state: EditorState; onClose?: () => void; embedded?: boolean }) {
  const content = <>
  ...
  if (embedded) return <div className="layers-panel embedded-layers">{content}</div>

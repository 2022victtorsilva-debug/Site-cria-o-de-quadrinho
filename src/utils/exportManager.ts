...
async function renderPage(page: ComicPage | { canvas: Record<string, unknown>; width: number; height: number }, format: 'png' | 'jpeg' = 'png', background = '#ffffff') {
  const { Canvas } = await import('fabric')
  const element = document.createElement('canvas')
  const canvas = new Canvas(element, { width: page.width, height: page.height, backgroundColor: background })
...
export async function exportStoredProject(project: Project, singlePageId?: string) {
  ...
  if (project.type === 'drawing') {
    const data = await renderPage({ canvas: project.project_data.canvas || { objects: [] }, width: project.project_data.width, height: project.project_data.height }, 'png', project.project_data.background)
  ...
  if (singlePageId) {
    ...
    if (page) downloadDataUrl(await renderPage(page, 'png', project.project_data.background), `${safeName}-${page.name}.png`)
    return
  }
  ...
  for (let index = 0; index < pages.length; index++) {
    ...
    const data = await renderPage(pages[index], 'jpeg', project.project_data.background)
